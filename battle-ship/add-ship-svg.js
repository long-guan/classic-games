...
export function displayShipSvg(startCoord, ship, yAxisMode) {
    let svgBoard = document.querySelector(".player1SvgBoard-square");
    let newDiv = document.createElement('div');
    if (yAxisMode == true) {
        newDiv.style.gridArea = `${parseInt(startCoord[0]) + 1} / ${parseInt(startCoord[1]) + 1} / span ${ship.length} / span 1`
    } else {
        newDiv.style.gridArea = `${parseInt(startCoord[0]) + 1} / ${parseInt(startCoord[1]) + 1} / span 1 / span ${ship.length}`
    }
    newDiv.appendChild(returnShipSvg(ship.name, yAxisMode));
    svgBoard.appendChild(newDiv);
    removePlaceShipListeners(player1Board);
}

function returnShipSvg(shipName, yAxisMode) {
    ...
    if (yAxisMode == true) {
        iconSvg.setAttribute('viewBox', shipAttributes[shipName]['viewbox']['y']);
        iconPath.setAttribute('transform', "" + shipAttributes[shipName]['rotate'])
    } else {
        iconSvg.setAttribute('viewBox', shipAttributes[shipName]['viewbox']['x']);
    }
    iconPath.setAttribute('d', shipAttributes[shipName]['path']);
    iconSvg.classList.add("svg-ship");
    iconSvg.appendChild(iconPath);
    return iconSvg
}

function removePlaceShipListeners(gameboard) {
